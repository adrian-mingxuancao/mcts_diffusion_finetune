...
"""
MCTS Folding Ablation Runner for PDB_date Dataset

Runs folding ablation studies on PDB_date dataset:
...
Task: FOLDING (sequence → structure optimization)
Baseline: ESMFold predictions  
Metrics: RMSD and TM-score vs ground truth structures
"""
...
def predict_structure_with_esmfold(sequence: str) -> Optional[np.ndarray]:
    ...
    try:
        ...
        if positions.dim() == 5:  # (batch, extra_dim, seq_len, atoms, 3)
...
def load_pdb_sequences() -> Dict[str, str]:
    ...
    try:
        ...
        print(f"✅ Loaded {len(sequences)} PDB sequences from {reference_fasta}")
    except Exception as e:
...
def run_folding_ablation(sequence: str, structure_name: str, reference_coords: np.ndarray,
    ...
    """
    Run single folding ablation experiment on PDB structure.
    
    Args:
        sequence: Input amino acid sequence
        structure_name: Structure identifier
        reference_coords: Reference CA coordinates from PDB
    ...
    print(f"  🎯 Task: Folding optimization (PDB dataset)")
    ...
    # Calculate baseline metrics against PDB reference
    ...
    # Create baseline structure for MCTS
    # Convert ESMFold coordinates to structure tokens for MCTS
    ...
    if baseline_coords is not None:
        try:
            ...
            print(f"  🔄 Converting ESMFold coordinates to structure tokens...")
            ...
            if struct_tokens is not None and isinstance(struct_tokens, torch.Tensor):
                ...
                print(f"  ✅ Generated baseline structure tokens: {len(token_list)} tokens")
            else:
    ...
    baseline_structure = {
        ...
        'struct_seq': struct_seq_str,  # Use converted tokens
        'name': structure_name,
        'pdb_id': structure_name.replace('PDB ', '').split('_')[0],
        'chain_id': structure_name.replace('PDB ', '').split('_')[1] if '_' in structure_name else 'A'
    }
...
def main():
    """Main PDB folding ablation study function"""
    print("🧬 MCTS Folding Ablation Study - PDB_date Dataset Evaluation")
    print("=" * 60)
    ...
    # Load PDB data
    pdb_sequences = load_pdb_sequences()
            ...
            try:
                ...
                if structure:
                    ...
                    if ref_coords is not None:
                        test_pairs.append((idx, f"PDB {structure_name}", sequence, ref_coords))
    ...
    # Save results
    output_dir = "/net/scratch/caom/pdb_folding_evaluation_results"
    os.makedirs(output_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Save JSON results
    json_file = os.path.join(output_dir, f"mcts_folding_pdb_results_{timestamp}.json")
    with open(json_file, "w") as f:
        json.dump(results, f, indent=2)
    print(f"\n💾 Saved folding PDB results → {json_file}")
    ...
    summary_file = os.path.join(output_dir, f"mcts_folding_pdb_summary_{timestamp}.txt")
    try:
        with open(summary_file, "w") as f:
            f.write("MCTS Folding Ablation Summary - PDB_date Dataset\n")
            f.write("=" * 80 + "\n\n")
