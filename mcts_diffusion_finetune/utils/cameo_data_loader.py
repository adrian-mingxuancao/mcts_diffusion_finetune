...
class CAMEODataLoader:
    ...
    def _convert_to_framework_format(self, cameo_data: Dict, structure_id: str) -> Dict:
            ...
            for cameo_struct_fasta in candidate_fastas:
                ...
                for record in SeqIO.parse(cameo_struct_fasta, "fasta"):
                    ...
                    if (rid in ids_to_try) or any(x in rid for x in ids_to_try):
                        ...
                        # CRITICAL: Store the original comma-separated string for DPLM2 tokenizer
                        # The tokenizer expects the exact format from FASTA (with commas, possibly newlines)
                        struct_seq_from_fasta = raw
                        
                        # Also parse to get struct_ids array for other uses
                        ...
                        print(f"   🏗️ ✅ MATCHED {rid} → struct_seq (original): {len(struct_seq_from_fasta)} chars")
                        break
        ...
        # Prefer struct_seq from FASTA (original format), then struct_ids
        final_struct_ids = None
        final_struct_seq = None
        
        if 'struct_seq_from_fasta' in locals() and struct_seq_from_fasta:
            # Use the original FASTA format (with commas and newlines preserved)
            final_struct_seq = str(struct_seq_from_fasta)
            print(f"   🏗️ Using struct_seq from struct.fasta: {len(final_struct_seq)} chars (original format)")
            # Also store struct_ids if we parsed them
            if 'struct_ids_from_fasta' in locals() and struct_ids_from_fasta:
                ...
                print(f"   🏗️ Also stored struct_ids array: {len(final_struct_ids)} tokens")
        elif struct_ids:
