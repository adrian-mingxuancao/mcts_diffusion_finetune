...
#SBATCH --output=/home/caom/AID3/dplm/logs/mcts_folding_ablation/mcts_fold_pdb_%A_%a.out
#SBATCH --error=/home/caom/AID3/dplm/logs/mcts_folding_ablation/mcts_fold_pdb_%A_%a.err

set -euo pipefail

echo "🚀 Starting MCTS Folding PDB Ablation Study"
echo "📅 $(date)"
...
# Build command - use PDB script
CMD="python mcts_folding_ablation_pdb.py ${START_IDX} ${END_IDX} --mode ${MODE} --num_iterations ${NUM_ITERATIONS} --max_depth ${MAX_DEPTH} --temperature ${TEMPERATURE} ${UCT_FLAG}"
if [ -n "${EXPERT_ID}" ]; then
