...
class RealProteinMPNNExpert:
    ...
    def generate(self, masked_sequence: str = None, num_samples: int = 1, 
        ...
        """Wrapper for generate_sequences."""
        return self.generate_sequences(masked_sequence, num_samples, structure_coords)
    
    def compute_entropy(self, masked_sequence: str, structure_coords: np.ndarray) -> float:
        """
        Compute real entropy for masked positions using ProteinMPNN model probabilities.
        
        Uses the sample() method to get probability distributions, then computes
        Shannon entropy: H = -Σ p(aa) * log(p(aa)) for masked positions.
        
        Args:
            masked_sequence: Sequence with 'X' at positions to compute entropy for
            structure_coords: CA coordinates [seq_len, 3]
        ...
        try:
            ...
            # Ensure shape is [seq_len, 3] for CA-only
            if len(coords.shape) == 3 and coords.shape[1] == 3 and coords.shape[2] == 3:
                coords = coords[:, 1, :]  # Extract CA atoms
            elif len(coords.shape) != 2 or coords.shape[1] != 3:
                raise ValueError(f"Expected coords shape [seq_len, 3], got {coords.shape}")
            ...
            # Create randn for decoding order
            randn = torch.randn(chain_M.shape, device=self.device)
            
            # Dummy parameters for sample method
            chain_M_pos = chain_M
            omit_AA_mask = torch.zeros(1, seq_len, 21, dtype=torch.float32, device=self.device)
            pssm_coef = torch.zeros(1, seq_len, dtype=torch.float32, device=self.device)
            pssm_bias = torch.zeros(1, seq_len, 21, dtype=torch.float32, device=self.device)
            ...
            with torch.no_grad():
                # Call sample to get probability distributions
                # The sample method returns a dictionary with keys: "S", "probs", "decoding_order"
                ...
                all_probs = output_dict["probs"]  # shape: [batch=1, seq_len, 21]
                
                # Compute entropy for masked positions: H = -Σ p * log(p)
                total_entropy = 0.0
                for pos in mask_positions:
                    if pos < all_probs.shape[1]:
                        probs = all_probs[0, pos, :]  # [21]
            ...
            if len(mask_positions) == 0:
                return 0.0
            masking_ratio = len(mask_positions) / len(masked_sequence)
            return min(2.0 * (0.5 + masking_ratio), 3.0)
