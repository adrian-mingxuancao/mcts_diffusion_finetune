...
class CAMEODataLoader:
    ...
    def _convert_to_framework_format(self, cameo_data: Dict, structure_id: str) -> Dict:
        ...
        if 'atom_positions' in cameo_data:
            coords = np.array(cameo_data['atom_positions'])
        ...
        if 'sequence' in cameo_data:
            ...
            # Convert amino acid indices to sequence
            sequence = self._aatype_to_sequence(cameo_data['aatype'])
        else:
            sequence = None
        ...
        if sequence:
            ...
            # 🎯 CHECK FOR TOKENS: Look for token patterns
            ...
            if start_offset > 0 or end_offset > 0:
                ...
                if 'coordinates' in cameo_data or 'atom_positions' in cameo_data:
                    ...
                    if coords.shape[0] == original_length:
                        # Remove token positions from coordinates
                        coords = coords[start_offset:coords.shape[0]-end_offset, :, :]
                        print(f"🎯 Aligned coordinates: {coords.shape[0]} residues (removed token positions)")
                    else:
                        print(f"⚠️ Warning: Coordinate length ({coords.shape[0]}) doesn't match sequence length ({original_length})")
        ...
        # Ensure coordinates are in correct format [L, 3, 3] (N, CA, C atoms)
        coords = self._ensure_backbone_format(coords)
        ...
        if sequence:
            ...
            if coords.shape[0] != actual_length:
                print(f"⚠️ WARNING: Coordinate length ({coords.shape[0]}) doesn't match sequence length ({actual_length})")
                if coords.shape[0] > actual_length:
                    # Truncate coordinates to match sequence
                    coords = coords[:actual_length, :, :]
                    print(f"🎯 Truncated coordinates from {coords.shape[0] + (coords.shape[0] - actual_length)} to {actual_length}")
                else:
                    # Pad coordinates to match sequence (this shouldn't happen with CAMEO data)
                    padding_needed = actual_length - coords.shape[0]
                    last_coord = coords[-1:] if coords.shape[0] > 0 else np.zeros((1, 3, 3))
                    padding = np.repeat(last_coord, padding_needed, axis=0)
                    coords = np.concatenate([coords, padding], axis=0)
                    print(f"🎯 Padded coordinates from {coords.shape[0] - padding_needed} to {actual_length}")
        else:
            # No sequence available, use coordinate length
            actual_length = coords.shape[0]
    ...
    def _aatype_to_sequence(self, aatype: List[int]) -> str:
        ...
        aa_map = {
            ...
            16: 'T', 17: 'W', 18: 'Y', 19: 'V', 20: 'X'  # X for unknown
        }
        return ''.join([aa_map.get(aa, 'X') for aa in aatype])
