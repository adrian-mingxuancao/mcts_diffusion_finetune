...
class BoltzDesignIntegration:
    ...
    def _real_design(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"      (This may take 10-30 minutes for diffusion sampling)")
            
            env = os.environ.copy()
            env['CUDA_VISIBLE_DEVICES'] = '0'
            # Add boltzdesign directory to PYTHONPATH for imports
            boltzdesign_dir = str(self.BOLTZDESIGN_PATH / "boltzdesign")
            env['PYTHONPATH'] = f"{boltzdesign_dir}:{env.get('PYTHONPATH', '')}"
            
            # Use Popen for streaming output so user can see progress
